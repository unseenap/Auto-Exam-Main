...
final class SchedulingValidator
{
    ...
    public function validate(array $scope, int $userId, bool $persist = true): array
    {
        ...
        $required=count($curriculum);$available=$examDays*min($shifts,$maximumPerDay);$largest=max($expected?:[0]);
        $available>=$largest?$add('Feasibility','pass','slots.capacity',"{$available} slots are available independently for each branch/semester. Different branches may schedule different subjects in the same session ({$required} branch-subject entries total)."):$add('Feasibility','blocked','slots.capacity',"Only {$available} slots per branch/semester for {$largest} required papers.");
        ...
        $gapSlots>=$largest?$add('Feasibility','pass','gap.window','Enabled dates can accommodate the largest curriculum with the requested calendar-day gap.'):$add('Feasibility','blocked','gap.window',"Only {$gapSlots} separated dates are available for {$largest} papers. Reduce the gap or extend the calendar.");
