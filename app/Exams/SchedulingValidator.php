...
final class SchedulingValidator
{
    ...
    public function validate(array $scope, int $userId, bool $persist = true): array
    {
        ...
        $requestedBatchIds=array_values(array_unique(array_filter(array_map('intval',$scope['batch_ids']??[]))));
        ...
        $validBatchIds=array_map('intval',array_column($cycleBatchRows,'id'));$invalidBatchIds=array_diff($requestedBatchIds,$validBatchIds);
        if(!$cycleBatchRows)$add('Academic','warning','scope.batches','This is a legacy examination cycle without batch assignments. New cycles require batches before creation.');elseif($outsideBatches)$add('Academic','blocked','scope.batches','One or more selected programmes do not have an assigned batch in this examination cycle.');elseif(!$requestedBatchIds)$add('Academic','blocked','scope.batches','Select at least one academic batch before choosing subjects.');elseif($invalidBatchIds)$add('Academic','blocked','scope.batches','One or more selected batches do not belong to this examination cycle.');
        $selectedBatches=array_values(array_filter($cycleBatchRows,static fn(array $batch):bool=>in_array((int)$batch['programme_id'],$programmeIds,true)&&in_array((int)$batch['id'],$requestedBatchIds,true)));$selectedBatchIds=array_map('intval',array_column($selectedBatches,'id'));
        $batchProgrammeMismatch=count($selectedBatches)!==count($requestedBatchIds);
        if($batchProgrammeMismatch)$add('Academic','blocked','scope.batch_programmes','Every selected batch must belong to one of the selected programmes.');
        elseif($cycleBatchRows&&$requestedBatchIds&&!$outsideBatches&&!$invalidBatchIds)$add('Academic','pass','scope.batches',count($selectedBatches).' selected student batch'.(count($selectedBatches)===1?' is':'es are').' scheduled independently.');
