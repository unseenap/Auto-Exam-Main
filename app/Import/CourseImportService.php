...
final class CourseImportService
{
    ...
    public function stage(string $path, string $originalName, int $userId): int
    {
        ...
        foreach($rows as $index=>$values){$source=[];foreach($headers as $column=>$header)if($header!=='')$source[$header]=trim((string)($values[$column]??''));$messages=[];
            ...
            $data=['code'=>$code,'name'=>$name,'programme_id'=>(int)($programme['id']??0),'programme_code'=>$programmeCode,'semester'=>$semester,'category'=>$category,'status'=>$status,'subject_priority'=>$priority,'course_credits'=>$credits,'course_level'=>$level,'course_year'=>$year,'mid_sem_duration_minutes'=>$midMinutes,'end_sem_duration_minutes'=>$endMinutes];
    ...
    public function commit(int $batchId,int $userId): int
    {
        ...
        foreach($q as $row){$data=json_decode($row['normalized_data'],true,flags:JSON_THROW_ON_ERROR);$find=$this->pdo->prepare('SELECT id FROM courses WHERE code=:code');$find->execute(['code'=>$data['code']]);$courseId=(int)$find->fetchColumn();
            ...
            $map=$this->pdo->prepare('INSERT INTO programme_courses(programme_id,course_id,semester,category,course_credits,course_level,course_year,mid_sem_duration_minutes,end_sem_duration_minutes,subject_priority) VALUES(:programme,:course,:semester,:category,:credits,:level,:year,:mid,:end,:priority) ON DUPLICATE KEY UPDATE category=VALUES(category),course_credits=VALUES(course_credits),course_level=VALUES(course_level),course_year=VALUES(course_year),mid_sem_duration_minutes=VALUES(mid_sem_duration_minutes),end_sem_duration_minutes=VALUES(end_sem_duration_minutes),subject_priority=VALUES(subject_priority)');$map->execute(['programme'=>$data['programme_id'],'course'=>$courseId,'semester'=>$data['semester'],'category'=>$data['category'],'credits'=>$data['course_credits'],'level'=>$data['course_level'],'year'=>$data['course_year'],'mid'=>$data['mid_sem_duration_minutes'],'end'=>$data['end_sem_duration_minutes'],'priority'=>$data['subject_priority']]);
